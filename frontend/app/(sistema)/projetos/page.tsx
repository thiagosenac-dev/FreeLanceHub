...
const LABELS: Record<string, string> = { ABERTO: "Aberto", EM_ANDAMENTO: "Em Andamento", CONCLUIDO: "Concluído", CANCELADO: "Cancelado" };
const STYLES: Record<string, string> = {
    ABERTO: "bg-blue-500/10 text-blue-400 border border-blue-500/20",
    EM_ANDAMENTO: "bg-amber-500/10 text-amber-400 border border-amber-500/20",
...
export default function Projetos() {
    ...
    const carregarDados = async () => {
        try { setProjetos((await axios.get<Projeto[]>("http://localhost:8080/projetos")).data) }
    ...
    const excluirProjeto = async (id: number | null) => {
        ...
        } catch (error) { console.error(error); alert("Erro ao excluir projeto") }
    }
            ...
            <div className="w-full max-w-6xl mx-auto relative z-10 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div className="bg-slate-950/80 backdrop-blur-xl border border-slate-800/80 rounded-xl p-4 shadow-xl">
                        ...
                        <p className="text-2xl font-bold text-blue-400 mt-1">{projetos.filter(p => String(p.status).toUpperCase() === 'EM_ANDAMENTO').length}</p>
                    </div>
                    <div className="bg-slate-950/80 backdrop-blur-xl border border-slate-800/80 rounded-xl p-4 shadow-xl">
                        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Concluídos</span>
                        <p className="text-2xl font-bold text-emerald-400 mt-1">{projetos.filter(p => String(p.status).toUpperCase() === 'CONCLUIDO').length}</p>
                    </div>
                </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="divide-y divide-slate-800/60 text-sm">
                                {projetos.map(p => {
                                    const status = String(p.status ?? "ABERTO").toUpperCase();
                                    return (
                                        <tr key={p.id} className="hover:bg-slate-900/30 transition-colors">
                                            ...
                                            <td className="py-4 px-6 text-slate-400">{fmt(p.valor)}</td>
                                            <td className="py-4 px-6 text-slate-400">{p.prazo}</td>
                                            <td className="py-4 px-6">
                                                <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${STYLES[status] ?? "bg-slate-500/10 text-slate-400 border border-slate-500/20"}`}>
                                                    {LABELS[status] ?? p.status ?? "Aberto"}
                                                </span>
                                            </td>
                                            <td className="py-4 px-6 text-right">
                                                <div className="flex items-center justify-end gap-3">
                                                    <Link href={`/projetos/${p.id}`} className="text-blue-400 hover:text-blue-300 font-medium text-xs transition-colors">Editar</Link>
                                                    <button onClick={() => excluirProjeto(p.id)} className="text-rose-400 hover:text-rose-300 font-medium text-xs transition-colors cursor-pointer">Excluir</button>
                                                </div>
                                            </td>
                                        </tr>
                                    );
                                ...
                                {projetos.length === 0 && (
                                    <tr><td colSpan={5} className="py-12 text-center text-slate-500 italic">Nenhum projeto encontrado</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>

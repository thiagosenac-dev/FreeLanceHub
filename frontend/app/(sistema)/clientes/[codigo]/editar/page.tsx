...
export default function EditarCliente(){
    // pega o código do cliente que vem na rota
    const codigo = Number(useParams().codigo);
    // busca o nome do cliente pra mostrar no título
    const [nome, setNome] = useState("");
    useEffect(() => { axios.get(`http://localhost:8080/clientes/${codigo}`).then(({ data }) => setNome(data.nome)).catch(() => {}); }, [codigo]);
                ...
                <div className={`${cardClasses} flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-6`}>
                    <div className="space-y-1.5">
                        <h1 className="flex items-center gap-3 text-2xl sm:text-3xl font-bold tracking-tight">
                            ...
                            <span className="bg-gradient-to-r from-blue-300 via-sky-300 to-cyan-300 bg-clip-text text-transparent">Editar Cliente: {nome}, Codigo: {codigo}</span>
                        </h1>

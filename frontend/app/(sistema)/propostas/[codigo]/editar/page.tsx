...
export default function EditarProposta(){
    // pega o código da proposta que vem na rota
    const codigo = Number(useParams().codigo);
    // busca a descrição da proposta pra mostrar no título
    const [descricao, setDescricao] = useState("");
    useEffect(() => { axios.get(`http://localhost:8080/propostas/${codigo}`).then(({ data }) => setDescricao(data.descricao)).catch(() => {}); }, [codigo]);

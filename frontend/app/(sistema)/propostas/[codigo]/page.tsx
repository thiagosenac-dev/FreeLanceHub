...
import PropostaForm from "../componentes/PropostaForm";

export default function EditarProposta(){

    // Pega o código da proposta que veio na URL (ex: /propostas/5)
    const parametro = useParams();

    const codigo = Number(parametro.codigo);

    // Guarda os dados da proposta carregada. Começa vazio (null) até a busca terminar
    const [proposta, setProposta] = useState<Proposta|null>(null)
    const router = useRouter();

    // Assim que a página abre, busca os dados dessa proposta
    useEffect(()=>{
    ...
    // Busca a proposta pelo código na API. Se não achar, volta para a listagem
    const buscarDados = async () => {

        const valorPropostaBack = await axios.get<Proposta>('http://localhost:8080/propostas/' + codigo);

        if(valorPropostaBack.status==200){
            setProposta(valorPropostaBack.data);
        }else{
    ...
    return(
        <div className="space-y-6">
            ...
            {/* Cabeçalho com título (mostrando o código da proposta) e botão de voltar */}
                ...
                <div className="space-y-1">
                    <h1 className="text-2xl font-bold tracking-tight text-white flex items-center space-x-2">
                        ...
                        <span className="bg-gradient-to-r from-blue-300 via-sky-300 to-cyan-300 bg-clip-text text-transparent">
                            Editar Proposta {codigo}
                        </span>
                    </h1>
                    <p className="text-sm text-slate-400">Preencha os dados para editar a Proposta</p>
                </div>
                <Link href="/propostas" className="inline-flex items-center justify-center text-sm font-medium text-slate-300 hover:text-white bg-white/[0.03] hover:bg-gradient-to-r hover:from-blue-600/20 hover:to-cyan-500/10 border border-white/[0.08] hover:border-blue-400/40 px-4 py-2.5 rounded-xl transition-all duration-300 hover:shadow-[0_0_18px_rgba(34,211,238,0.3)] hover:-translate-x-0.5 w-full sm:w-auto">
                    &larr; Voltar para Listagem
                </Link>
            </div>

            {/* Card com o formulário já preenchido com os dados da proposta buscada acima */}
            <div className="bg-slate-950/70 backdrop-blur-xl border border-white/[0.06] rounded-2xl p-6 md:p-8 shadow-[0_8px_30px_-8px_rgba(3,10,28,0.8)]">
                <PropostaForm propostaExistente={proposta}/>
            </div>
        </div>
    )
